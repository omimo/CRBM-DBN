...
def preprocess_data_ms3(Motion):

    n_seq = Motion.shape[1]

    # collapse sequences
    batchdata = np.concatenate([m for m in Motion.flat], axis=0)
...
def load_data_ms3(filename):

    # load data post preprocess1
    mat_dict = scipy.io.loadmat(filename)
    Motion = mat_dict['Motion']

    batchdata, seqlen, data_mean, data_std = preprocess_data_ms3(Motion)
